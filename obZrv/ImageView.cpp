...
int ImageView::zoom(int inout, bool test)
{
    ...
    if (inout > 0)
    {
        ...
        if (test)
            return 0;
        _zoomlevel = *std::upper_bound(levels.begin(), levels.end(),
            _zoomlevel != 0 ? _zoomlevel : _fitlevel);
    }
    else
    {
        if (_zoomlevel != 0 && _zoomlevel <= levels.front())
            return -1;
        int newlevel = *std::upper_bound(levels.rbegin(), levels.rend(),
            _zoomlevel != 0 ? _zoomlevel : _fitlevel, std::greater<int>());
