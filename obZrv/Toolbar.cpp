...
/* PNG strip has 9 icons; FileList and OpenRemote are appended after */
#define IMG_OPEN        0
#define IMG_PREV        1
#define IMG_NEXT        2
#define IMG_ZOOMIN      3
...
#define IMG_FILELIST    9   /* shell32 folder, appended after the strip */
#define IMG_OPENREMOTE  10  /* shell32 network, appended after filelist */
...
bool Toolbar::Create(HWND hParent, HINSTANCE hInst)
{
    ...
    /* Button layout:
     *  [Open][Prev][Next] | [ZoomIn][ZoomTo][ZoomMode][ZoomOut][ZoomRem][Zoom] | [FileList(toggle)]
...
void Toolbar::UpdateDpi()
{
    ...
    /* Append the FileList (folder) and OpenRemote (network) shell32 icons */
    if (himlNew) {
        HICON hFolder = LoadShell32Icon(SHELL32_FOLDER, iconSz);
        if (hFolder) {
            ImageList_AddIcon(himlNew, hFolder);
            if (himlNewDis) ImageList_AddIcon(himlNewDis, hFolder);
            DestroyIcon(hFolder);
        }
        HICON hNetwork = LoadShell32Icon(SHELL32_NETWORK, iconSz);
        if (hNetwork) {
            ImageList_AddIcon(himlNew, hNetwork);
            if (himlNewDis) ImageList_AddIcon(himlNewDis, hNetwork);
            DestroyIcon(hNetwork);
        }
    }
