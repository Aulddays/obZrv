...
void *BasicBitmap::LoadContent(const char *filename, long *size)
{
	...
	return LoadContent(fp, size);
#else
	if (size) size[0] = -1;
	return NULL;
#endif
}
void *BasicBitmap::LoadContent(const wchar_t *filename, long *size)
{
#ifndef PIXEL_NO_STDIO
...
void *BasicBitmap::LoadContent(FILE *fp, long *size)
{
#ifndef PIXEL_NO_STDIO
	...
	int sw, int sh)
{
	...
	//void *tmpbits = NULL;
	//HBITMAP hBitmap = CreateDIB(hDC, int w, int h, PixelFmt fmt, void **bits)
	//BitBlt(hDC, )
...
BasicBitmap *BasicBitmap::GdiPlusLoadImage(const char *filename, PixelFmt fmt)
{
	...
	content = LoadContent(filename, &size);
	if (content == NULL) return NULL;
	BasicBitmap *bmp = GdiPlusLoadImage(content, size, fmt);
	free(content);
	return bmp;
}
BasicBitmap *BasicBitmap::GdiPlusLoadImage(const wchar_t *filename, PixelFmt fmt)
{
	void *content;
	long size;
	content = LoadContent(filename, &size);
...
BasicBitmap *BasicBitmap::GdiPlusLoadImage(const void *content, long size, PixelFmt fmt)
{
	if (content == NULL) return NULL;
